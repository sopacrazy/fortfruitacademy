...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Load Content
      const savedModules = storageService.getModules();
  ...
  const handleAddContent = (moduleId: ModuleId, type: 'video' | 'document', data: any) => {
    // Save to "Backend"
    const updatedModules = storageService.addContentToModule(moduleId, type, data);
    // Update UI
      ...
      {view === 'player' && getCurrentModule() ? (
        ...
        <div className="flex flex-col h-full overflow-y-auto">
            ...
            <div className="max-w-7xl mx-auto px-8 flex justify-between items-center">
              ...
              >
                ...
                <span className="font-bold text-xl tracking-tight text-gray-800">
                  Fort Fruit <span className="text-fort-600 font-normal">Aprendizado</span>
                </span>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-semibold text-gray-800 uppercase">{currentUser || 'Admin'}</p>
          ...
          <footer className="bg-white border-t border-gray-200 mt-auto py-8 text-center text-gray-400 text-sm shrink-0">
            <p>&copy; {new Date().getFullYear()} Fort Fruit. Todos os direitos reservados.</p>
